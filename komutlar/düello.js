...
exports.run = async (client, message, args) => {
  ...
  this.fighting = new Set();
	...
	if (!opponent) return message.reply("**Oynamak istediğin kişiyi etiketlemelisin** <a:memedance:814085110407495780>")
  ...
  if (opponent.id === message.author.id) return message.reply('Kendin ile düello Atamazsın');
		if (this.fighting.has(message.channel.id)) return message.reply('<a:memedance:814085110407495780> | **Kanal başına sadece bir düello meydana gelebilir.**');
		this.fighting.add(message.channel.id);
		try {
			if (!opponent.bot) {
                await message.channel.send(`<a:memedance:814085110407495780> ${opponent}, **düello isteği geldi.** **Düello'yu kabul ediyor musun?** (\`evet\` veya \`hayir\` **olarak cevap veriniz.**)`);
